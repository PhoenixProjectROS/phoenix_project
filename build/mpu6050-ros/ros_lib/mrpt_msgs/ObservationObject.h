#ifndef _ROS_mrpt_msgs_ObservationObject_h
#define _ROS_mrpt_msgs_ObservationObject_h
...
#include "mrpt_msgs/SingleObjectObservation.h"

namespace mrpt_msgs
{

  class ObservationObject : public ros::Msg
  {
    public:
      ...
      typedef mrpt_msgs::SingleObjectObservation _sensed_data_type;
    ...
    virtual int deserialize(unsigned char *inbuffer)
    {
      ...
      if(sensed_data_lengthT > sensed_data_length)
        this->sensed_data = (mrpt_msgs::SingleObjectObservation*)realloc(this->sensed_data, sensed_data_lengthT * sizeof(mrpt_msgs::SingleObjectObservation));
      sensed_data_length = sensed_data_lengthT;
      for( uint32_t i = 0; i < sensed_data_length; i++){
      offset += this->st_sensed_data.deserialize(inbuffer + offset);
        memcpy( &(this->sensed_data[i]), &(this->st_sensed_data), sizeof(mrpt_msgs::SingleObjectObservation));
      }
     return offset;
    }

    const char * getType(){ return "mrpt_msgs/ObservationObject"; };
    const char * getMD5(){ return "69850522dc16b0669d88443ddbccc410"; };
